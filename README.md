...
Spring Boot project with a layered architecture (controller, service, repository), request validation, centralized error handling, automated tests, OpenAPI documentation, optional PostgreSQL support via Docker, and an AI summary endpoint powered by Google Gemini.
...
**Requirements:** JDK 25, Maven (or IntelliJ). Docker is required only for the PostgreSQL setup. The AI endpoint requires a `GEMINI_API_KEY` environment variable.
...
| POST | `/api/applications/{id}/ai/summary` | AI summary and follow-up suggestion |
...
`POST /api/applications/{id}/ai/summary` reads an application from the database and returns a short summary with a follow-up suggestion, generated by Google Gemini through Spring AI.

Set the API key as an environment variable before starting the application:

```
GEMINI_API_KEY=your-key-here
```

A free-tier key is available from [Google AI Studio](https://aistudio.google.com/apikey). Do not commit the key to the repository.

In IntelliJ: Run Configuration → Environment variables → add `GEMINI_API_KEY`.
